...
### ③ 동시호가 & 단일가 체결 관리 (After-Hours Order Management)
- **장 마감 후 예약 주문 관리**: 장 마감 시간 이후 접수된 동시호가 예약 주문을 일괄 조회.
- **단일가 일괄 체결 RPC 연동**: 시장 재개장 시 `execute_batch_single_price_orders` RPC를 통해 대기 예약 주문을 단일가로 원자적 일괄 체결, 결과를 관리자 화면에 즉시 표시.
